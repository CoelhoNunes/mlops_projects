...
def load_customer_data(
    data_path: str, smoke: bool = False
...
def engineer_features(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    df_engineered["zip_code_numeric"] = pd.to_numeric(
        df_engineered["customer_zip_code_prefix"], errors="coerce"
    )

    # 2. Create region features based on zip code ranges
    df_engineered["region"] = df_engineered["zip_code_numeric"].apply(
        lambda x: (
            "south"
            if x >= 80000
            else (
                ...
                if x >= 20000
                    ...
                    if x >= 40000
                        ...
                        "north"
                        if x >= 60000
                        else "midwest" if x >= 70000 else "unknown"
                    )
                )
            )
        )
    ...
    df_engineered["customer_id_hash"] = (
        df_engineered["customer_unique_id"].apply(hash) % 1000
    )
    ...
    df_engineered["city_encoded"] = le_city.fit_transform(
        df_engineered["customer_city"]
    )

    # 6. Create interaction features
    df_engineered["zip_region_interaction"] = (
        df_engineered["zip_code_numeric"] * df_engineered["region_encoded"]
    )
    ...
    encoders = {"region_encoder": le_region, "city_encoder": le_city}
...
def split_data(
    X: pd.DataFrame, y: pd.Series, test_size: float = 0.2, val_size: float = 0.2
) -> Tuple:
...
def create_lr_pipeline_with_params(penalty: str) -> Pipeline:
    ...
    if penalty == "l1":
        ...
        model = LogisticRegression(
            random_state=42, max_iter=1000, n_jobs=-1, solver="liblinear"
        )
    else:
...
def train_models(
    ...
    study_rf.optimize(
        lambda trial: objective(trial, X_train, y_train, X_val, y_val, "rf"),
        n_trials=20,
    )
    ...
    study_lr.optimize(
        lambda trial: objective(trial, X_train, y_train, X_val, y_val, "lr"),
        n_trials=20,
    )
...
def create_confusion_matrix_plot(cm: np.ndarray, save_path: str):
    ...
    sns.heatmap(
        cm, annot=True, fmt="d", cmap="Blues", xticklabels=True, yticklabels=True
    )
...
def log_to_mlflow(
    ...
    # Set experiment name
    experiment_name = (
        "customer-classification-smoke" if smoke else "customer-classification"
    )
    ...
    with mlflow.start_run():
        ...
        mlflow.log_params(
            {
                ...
                "smoke_test": smoke,
                "n_features": (
                    model.named_steps["classifier"].n_features_in_
                    if hasattr(model.named_steps["classifier"], "n_features_in_")
                    else "unknown"
                ),
                "random_state": 42,
            }
        ...
        if training_results["best_model_name"] == "random_forest":
            ...
            mlflow.log_params(
                training_results["models"]["logistic_regression"]["params"]
            )
        ...
        mlflow.log_param(
            "feature_names",
            list(X_test.columns) if hasattr(X_test, "columns") else "unknown",
        )
        ...
        for name, encoder in encoders.items():
            mlflow.sklearn.log_model(
                encoder, name, registered_model_name=f"customer-classification-{name}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Customer Classification Training Pipeline"
    )
    parser.add_argument(
        "--smoke", action="store_true", help="Run smoke test with small dataset"
    )
    parser.add_argument(
        "--data-path",
        type=str,
        default="./customer_dataset.csv",
    ...
    try:
        ...
        evaluation_results = evaluate_model(
            training_results["best_model"], X_test, y_test
        )

        # Create confusion matrix plot
        confusion_matrix_path = "customer_confusion_matrix.png"
        create_confusion_matrix_plot(
            np.array(evaluation_results["confusion_matrix"]), confusion_matrix_path
        )
